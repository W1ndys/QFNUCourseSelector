...
这里只说明拿到登录成功后的 session 后的处理

## 登录成功后

获取选课系统的选课轮次编号，下面是一个示例函数
...
访问选课页面，来刷新新的 session，因为选课的 session 比初始登录后多了一些东西，把 session 覆盖到原来的 session

```py
session.get(f"http://zhjw.qfnu.edu.cn/jsxsd/xsxk/xsxk_index?jx0502zbid={current_jx0502zbid}")
```

## 依次从多个选课模块发送查询请求

下面是只获取查询结果里的选课需要用的两个 ID 值的函数，实际上查询 API 需要更多参数，我会在选课开放后给你全部参数数据
...
至此，预选课查询API的相关需求已经全部说明完毕

## 需求

你需要完成的是，前端请求一个必填参数课程名称或课程ID，其他可选参数：老师姓名、上课星期、上课节次

后端进行多个模块的请求遍历，返回给前端查询结果和查询结果所在的模块

要对多种http请求错误代码进行合理的错误信息反馈