...
## 2025年9月24日公告：本项目具有严重的安全风险和非预期运行，有极大概率无法正常的选课，为避免影响正常选课，请勿继续使用，相关代码仅供学习研究使用，请勿用于实际的选课环境中，使用脚本造成的一切后果与开发者无关
  ...
  "courses": [
    {
      ...
      "lab_preference": {
        // 选填，实验学时偏好设置（仅用于需要同时选择讲课学时和实验学时的课程）
        "week_day": "实验学时上课星期", // 选填(1-7)
        "weeks": "实验学时上课周次", // 选填
        "class_period": "实验学时上课节次" // 选填
      }
    }
    //...
...
| lab_preference    | 实验学时偏好设置               | ⭕       | 见下方详细说明                                |

#### 组合选课功能说明

本脚本支持自动识别和选择需要同时选择讲课学时和实验学时的课程。当系统检测到课程名称包含 `[讲课学时]` 或 `[实验学时]` 时，会自动进行以下处理：

**自动识别逻辑：**

1. 系统搜索课程时，如果发现同一个课程号、同一个老师有多个课程记录
2. 自动识别哪些是讲课学时，哪些是实验学时
3. 自动组合选择两个学时类型的课程

**实验学时偏好设置：**
当有多个实验学时时，可以通过 `lab_preference` 指定偏好：

```json
{
  "course_id_or_name": "301043",
  "teacher_name": "李媛媛",
  "week_day": "2", // 讲课学时的偏好时间
  "weeks": "1-12", // 讲课学时的偏好周次
  "class_period": "5-7", // 讲课学时的偏好节次
  "lab_preference": {
    // 实验学时的偏好设置
    "week_day": "6", // 偏好星期六
    "weeks": "1-18", // 偏好1-18周
    "class_period": "1-2" // 偏好1-2节
  }
}
```
...
1. 首先选择讲课学时课程
2. 然后同时选择讲课学时和实验学时课程（使用 `yxjx0404id` 参数）
3. 确保两个学时类型都能成功选择
...
> **组合选课配置建议：**
>
> 1. **只配置讲课学时信息**：系统会自动搜索和识别实验学时
> 2. **使用 lab_preference**：当有多个实验学时时，指定偏好的时间
> 3. **保持时间信息完整**：即使系统会自动搜索，建议保留时间信息作为偏好参考
...
> **新增功能：组合选课支持**
>
> 本脚本现在支持自动识别和选择需要同时选择讲课学时和实验学时的课程。当系统检测到课程名称包含 `[讲课学时]` 或 `[实验学时]` 时，会自动进行组合选课，无需手动配置两个课程的 ID。
