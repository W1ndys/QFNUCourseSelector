  ...
  "courses": [
    {
      ...
      "week_day": "上课星期", // 选填(1-7)，用于查询剩余容量
      "weeks": "上课周次", // 选填，支持多种格式，例如："1-12"、"1-12,13-14"、"1,3,5,7,9,11,13,15,17"，用于查询剩余容量
...
| jx02id            | 课程 jx02id（用于选课请求）            | ✅       | 需要手动获取，详见下方说明                    |
| jx0404id          | 课程 jx0404id（用于选课请求）          | ✅       | 需要手动获取，详见下方说明                    |
| week_day          | 上课星期（用于查询剩余容量）           | ⭕       | 1-7 之间的数字                                |
| weeks             | 上课周次（用于查询剩余容量）           | ⭕       | "1-12"、"1-12,13-14"、"1,3,5,7,9,11,13,15,17" |
| class_period      | 上课节次（用于查询剩余容量）           | ⭕       | 1-2,1-3,1-4,9-10,9-11                         |
...
> - `jx02id` 和 `jx0404id`：必填，用于实际的选课请求操作
> - `class_period`、`week_day` 和 `weeks`：选填，仅用于在选课前查询课程的剩余容量信息，不影响选课请求
>
> 这样的设计可以简化流程，提高选课速度，避免因搜索失败而影响选课。
